...
if (empty($errors)) {
    try {
        ...
        foreach ($trainResults as $train) {
            ...
            $stationsStmt = $pdo->prepare($stationsSql);
...
if (!empty($departureDate)) {
    $departureDateObj = new DateTime($departureDate);
    $formattedDepartureDate = $departureDateObj->format('D, M j, Y');
}

// Format the return date for display (if applicable)
$formattedReturnDate = '';
if ($tripType === 'roundTrip' && !empty($returnDate)) {
    $returnDateObj = new DateTime($returnDate);
    $formattedReturnDate = $returnDateObj->format('D, M j, Y');
}